 ...
 *	@date 21 Nov 2018
 *	@brief This .c file contains a function called SlowMotion
 *		   which is responsible to take a .wav file, make it
 *  	   sound slower and save it in a new .wav file
 */
...
public int SlowMotion(char* filename){
	...
	h->sample_rate = file->h->sample_rate/2;  //sampple rate is divided by 2 to become 2 times slower
	...
	int i;
	//transfers the bytes from the original to the new wav file
